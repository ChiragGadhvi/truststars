...
import { Star, GitFork, Users, Share2, ChevronRight, Globe } from 'lucide-react'
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: userData } = await supabase
        .from('users')
        .select('*')
        .eq('github_username', resolvedParams.username)
        .single()
      ...
      const reposList = userRepos?.map(ur => ur.repositories).filter(Boolean) || []
      setRepos(reposList)

      // Calculate aggregated stats
      const stars = reposList.reduce((sum, repo: any) => sum + (repo.stars || 0), 0)
      const forks = reposList.reduce((sum, repo: any) => sum + (repo.forks || 0), 0)
      setTotalStars(stars)
      setTotalForks(forks)
      setLoading(false)
    }
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button 
              variant="outline" 
              size="sm" 
              className="text-xs transition-all hover:scale-105 bg-background shadow-sm hover:shadow-md border-border/50"
              onClick={handleShare}
            >
